...
import { View, Text, ScrollView, StyleSheet, Image, TouchableOpacity, Dimensions, Linking, Alert } from 'react-native';
...
export default function PropertyDetail() {
  ...
  useEffect(() => {
    ...
    AsyncStorage.getItem('recently_viewed').then(val => {
      ...
      AsyncStorage.setItem('recently_viewed', JSON.stringify(updated)).catch(() => {});
    }).catch(() => {});
  ...
  useEffect(() => {
    ...
    withTimeout(
      ...
      .catch(() => { /* silent: just keeps current favorite state */ });
  }, [user, id]);
  ...
  const toggleFavorite = async () => {
    ...
    try {
      if (wasFavorite) {
        await withTimeout(supabase.from('favorites').delete().eq('user_id', user.id).eq('property_id', property.id));
      } else {
        await withTimeout(supabase.from('favorites').insert({ user_id: user.id, property_id: property.id }));
      }
  ...
  if (loading || error) {
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor: Colors.white, paddingTop: 8 }}>
        <DataState loading={loading} error={error} onRetry={loadProperty}>{null}</DataState>
      </SafeAreaView>
    );
  }
...
const styles = StyleSheet.create({
  ...
  price: { fontSize: 21, fontWeight: '800', color: Colors.orange },
  ...
  title: { fontSize: 18, fontWeight: '700', color: Colors.slate900, marginTop: 6 },
  locationRow: { flexDirection: 'row', alignItems: 'center', gap: 3, marginTop: 4 },
  location: { fontSize: 14, color: Colors.slate500 },
  ...
  featureText: { fontSize: 13, fontWeight: '600', color: Colors.slate700 },
  sectionTitle: { fontSize: 16, fontWeight: '700', color: Colors.slate900, marginTop: 18, marginBottom: 6 },
  description: { fontSize: 14, color: Colors.slate600, lineHeight: 21 },
  ...
  tagText: { fontSize: 13, color: Colors.slate700 },
