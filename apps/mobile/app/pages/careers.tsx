...
import { View, Text, FlatList, ScrollView, TextInput, StyleSheet, TouchableOpacity, ActivityIndicator, Alert } from 'react-native';
...
export default function CareersPage() {
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    const { data } = await supabase.from('jobs').insert({ ...form, status: 'active', tags: [] }).select().single();
    ...
    <View style={styles.container}>
      ...
      {showForm ? (
        ...
        <>
          ...
          {loading ? <ActivityIndicator size="small" color={Colors.orange} style={{ marginTop: 40 }} /> : (
...
const styles = StyleSheet.create({
  ...
  input: { borderWidth: 1, borderColor: Colors.slate200, borderRadius: 10, paddingHorizontal: 12, paddingVertical: 9, fontSize: 12, color: Colors.slate900, backgroundColor: Colors.slate50, marginBottom: 8 },
  ...
  typeText: { fontSize: 10, fontWeight: '500', color: Colors.slate600 },
  ...
  filterText: { fontSize: 9, fontWeight: '500', color: Colors.slate600 },
  filterTextActive: { color: Colors.white },
  card: { backgroundColor: Colors.white, padding: 12, borderRadius: 12, marginBottom: 10 },
  cardTitle: { fontSize: 12, fontWeight: '700', color: Colors.slate900, flex: 1 },
  badge: { backgroundColor: Colors.orangeLight + '30', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  badgeText: { fontSize: 9, fontWeight: '600', color: Colors.orange },
  cardCompany: { fontSize: 10, color: Colors.orange, marginTop: 3 },
  cardCity: { fontSize: 9, color: Colors.slate400, marginTop: 2 },
  cardSalary: { fontSize: 10, fontWeight: '600', color: Colors.slate700, marginTop: 3 },
  cardDesc: { fontSize: 10, color: Colors.slate500, marginTop: 4, lineHeight: 14 },
  empty: { textAlign: 'center', color: Colors.slate400, marginTop: 40, fontSize: 11 },
  ...
  ctaText: { fontSize: 10, color: Colors.slate500, flex: 1 },
  ctaBtn: { backgroundColor: Colors.orange, paddingHorizontal: 14, paddingVertical: 8, borderRadius: 8 },
  ctaBtnText: { fontSize: 10, fontWeight: '600', color: Colors.white },
});
