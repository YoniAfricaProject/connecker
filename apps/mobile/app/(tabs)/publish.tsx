...
export default function PublishTab() {
  ...
  if (!user) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.centerSub}>Vous devez etre connecte pour publier une annonce</Text>
  ...
  if (success) {
    return (
      <View style={styles.center}>
        ...
        <Text style={styles.centerTitle}>Annonce publiee !</Text>
        <Text style={styles.centerSub}>Elle sera visible apres validation par notre equipe</Text>
        <TouchableOpacity style={styles.primaryBtn} onPress={() => { setSuccess(false); setStep(1); setForm({ title: '', description: '', transaction_type: 'sale', property_type: 'apartment', price: '', surface_area: '', rooms: '', bedrooms: '', bathrooms: '', ville: 'Dakar', commune: '', district: '', address: '', features: [] }); setImages([]); }}>
          <Text style={styles.primaryBtnText}>Publier une autre</Text>
        </TouchableOpacity>
      </View>
    );
  }
  ...
  const handleSubmit = async () => {
    if (!form.title || !form.price || !form.address) {
      Alert.alert('Erreur', 'Titre, prix et adresse sont obligatoires');
      return;
    }
    setSubmitting(true);
    try {
      ...
      // Upload images using FormData (works on React Native)
      if (property && images.length > 0) {
        for (let i = 0; i < images.length; i++) {
          ...
          await fetch(`https://xkylbaiguwgpncyvfsyy.supabase.co/storage/v1/object/property-images/${path}`, {
            method: 'POST',
            headers: {
              'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
              'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhreWxiYWlndXdncG5jeXZmc3l5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU1NTIyMTksImV4cCI6MjA5MTEyODIxOX0.yywKlNp-Pl9KklJJ82XkxV6vWn1xrqAVOKz6Av7Iq0o',
            },
            body: formData,
          });

          const { data: { publicUrl } } = supabase.storage.from('property-images').getPublicUrl(path);
          await supabase.from('property_images').insert({ property_id: property.id, url: publicUrl, is_primary: i === 0, sort_order: i });
        }
      }

      // Upload Tektal voice note
      if (property && tektalUri) {
        ...
        await fetch(`https://xkylbaiguwgpncyvfsyy.supabase.co/storage/v1/object/property-images/${tektalPath}`, {
          method: 'POST',
          headers: {
            'Authorization': `Bearer ${(await supabase.auth.getSession()).data.session?.access_token}`,
            'apikey': 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6InhreWxiYWlndXdncG5jeXZmc3l5Iiwicm9sZSI6ImFub24iLCJpYXQiOjE3NzU1NTIyMTksImV4cCI6MjA5MTEyODIxOX0.yywKlNp-Pl9KklJJ82XkxV6vWn1xrqAVOKz6Av7Iq0o',
          ...
          body: formData,
        }).catch(() => {}); // Voice note upload is optional
      }
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ padding: 16 }}>
        {step === 1 && (
          <>
            ...
            <TextInput value={form.title} onChangeText={v => setForm({ ...form, title: v })} style={styles.input} placeholder="Ex: Belle villa avec piscine" placeholderTextColor={Colors.slate400} />

            <Text style={styles.label}>Transaction</Text>
            <View style={styles.toggleRow}>
              {[{ v: 'sale', l: 'Vente' }, { v: 'rent', l: 'Location' }].map(({ v, l }) => (
            ...
            <TextInput value={form.description} onChangeText={v => setForm({ ...form, description: v })} style={[styles.input, { height: 70, textAlignVertical: 'top' }]} placeholder="Decrivez votre bien..." multiline placeholderTextColor={Colors.slate400} />
        ...
        {step === 2 && (
          <>
            <Text style={styles.stepTitle}>Localisation & details</Text>
            ...
            <View style={styles.row}>
              <View style={styles.rowItem}><Text style={styles.label}>Surface m²</Text><TextInput value={form.surface_area} onChangeText={v => setForm({ ...form, surface_area: v })} style={styles.input} keyboardType="numeric" placeholder="120" placeholderTextColor={Colors.slate400} /></View>
              <View style={styles.rowItem}><Text style={styles.label}>Pieces</Text><TextInput value={form.rooms} onChangeText={v => setForm({ ...form, rooms: v })} style={styles.input} keyboardType="numeric" placeholder="4" placeholderTextColor={Colors.slate400} /></View>
            </View>
            <View style={styles.row}>
              <View style={styles.rowItem}><Text style={styles.label}>Chambres</Text><TextInput value={form.bedrooms} onChangeText={v => setForm({ ...form, bedrooms: v })} style={styles.input} keyboardType="numeric" placeholder="3" placeholderTextColor={Colors.slate400} /></View>
              <View style={styles.rowItem}><Text style={styles.label}>Sdb</Text><TextInput value={form.bathrooms} onChangeText={v => setForm({ ...form, bathrooms: v })} style={styles.input} keyboardType="numeric" placeholder="2" placeholderTextColor={Colors.slate400} /></View>
            </View>

            <View style={styles.navRow}>
              <TouchableOpacity style={styles.backBtn} onPress={() => setStep(1)}><Text style={styles.backBtnText}>Retour</Text></TouchableOpacity>
              <TouchableOpacity style={styles.nextBtn} onPress={() => setStep(3)}><Text style={styles.nextBtnText}>Suivant</Text><Ionicons name="arrow-forward" size={14} color={Colors.white} /></TouchableOpacity>
            </View>
          </>
        )}
        ...
        {step === 3 && (
          <>
            <Text style={styles.stepTitle}>Photos & equipements</Text>
            ...
            <View style={styles.navRow}>
              <TouchableOpacity style={styles.backBtn} onPress={() => setStep(2)}><Text style={styles.backBtnText}>Retour</Text></TouchableOpacity>
              <TouchableOpacity style={[styles.nextBtn, { flex: 2 }]} onPress={handleSubmit} disabled={submitting}>
                {submitting ? <ActivityIndicator color={Colors.white} size="small" /> : <><Ionicons name="checkmark-circle" size={14} color={Colors.white} /><Text style={styles.nextBtnText}>Publier</Text></>}
              </TouchableOpacity>
            </View>
          </>
        )}
...
const styles = StyleSheet.create({
  ...
  centerTitle: { fontSize: 15, fontWeight: '700', color: Colors.slate900, marginTop: 12 },
  centerSub: { fontSize: 10, color: Colors.slate500, textAlign: 'center', marginTop: 4 },
  ...
  stepText: { fontSize: 10, fontWeight: '600', color: Colors.slate400 },
  ...
  stepTitle: { fontSize: 14, fontWeight: '700', color: Colors.slate900, marginBottom: 12, marginTop: 4 },
  label: { fontSize: 9, fontWeight: '600', color: Colors.slate500, marginBottom: 4, marginTop: 12, textTransform: 'uppercase', letterSpacing: 0.5 },
  ...
  toggleBtn: { flex: 1, paddingVertical: 9, borderRadius: 8, alignItems: 'center', backgroundColor: Colors.slate100 },
  toggleActive: { backgroundColor: Colors.orange },
  toggleText: { fontSize: 10, fontWeight: '500', color: Colors.slate600 },
  toggleTextActive: { color: Colors.white, fontWeight: '600' },

  row: { flexDirection: 'row', gap: 10 },
  rowItem: { flex: 1 },

  photoBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderWidth: 1.5, borderStyle: 'dashed', borderColor: Colors.slate200, borderRadius: 12, paddingVertical: 16 },
  photoBtnText: { fontSize: 11, fontWeight: '500', color: Colors.slate500 },
  ...
  featuresWrap: { flexDirection: 'row', flexWrap: 'wrap', gap: 5 },
  featureChip: { paddingHorizontal: 10, paddingVertical: 6, borderRadius: 7, backgroundColor: Colors.slate100 },
  featureChipActive: { backgroundColor: Colors.orange },
  featureText: { fontSize: 9, fontWeight: '500', color: Colors.slate600 },
  ...
  backBtnText: { fontSize: 11, fontWeight: '500', color: Colors.slate600 },
  ...
  nextBtnText: { fontSize: 11, fontWeight: '600', color: Colors.white },
});
